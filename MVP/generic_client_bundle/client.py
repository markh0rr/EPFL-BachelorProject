...
def ping_function(token, server_unique_id, full_api_address, DEBUG):
    ...
    while True :
        ...
        try :
            answer = requests.post(f"{full_api_address}/post_server_feedback/", json=data, headers=headers, verify="./server.crt")
            if DEBUG :
                print(f"    [*] {answer.text}")
...
if server_unique_id != None and token != None and api_address != None : 
    full_api_address = f"https://{api_address}:9494"
    ...
    while True :
        try :
            ...
            ## fetch instruction 
            response = requests.post(f"{full_api_address}/get_tasks/", json=request_data, headers=headers, verify="./server.crt")
            ...
            if "task_id" in task.keys() and "task_json" in task.keys() :
                ...
                answer = requests.post(f"{full_api_address}/{fetch_from_url}", json=request_data, headers=headers, verify="./server.crt")
                ...
                answer = requests.post(f"{full_api_address}/post_task_feedback/", json=request_data, headers=headers, verify="./server.crt")
